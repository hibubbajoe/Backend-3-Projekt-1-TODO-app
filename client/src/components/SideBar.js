...
export default function SideBar() {
    ...
    const {categoryData, todoData, setFilteredTodos, setTrigger, trigger} = useContext(FetchContext);
    ...
    const insertCategory = async (e) => {
        ...
        try {
            await api.addNewUserCategory(newCategory);
            setInsertValue({})
            setCollapse(false);
            setTrigger(!trigger)
        } catch (error) {
    ...
    const deleteCategory = async (e, id) => {
        ...
        try {
            await api.deleteUserCategory(id);
            setTrigger(!trigger);
        } catch (error) {
            console.log(error)
        }
...
return (
    ...
    <Box xs={2} sx={{ position: 'absolute', top: '10%', display: 'flex', flexDirection: 'column', width: '15rem' }}>
        ...
        <Button sx={{ color: 'black', justifyContent: 'flex-start', pl: 3 }} onClick={() => setCollapse(!collapse)}><AddOutlinedIcon />Add category</Button>
